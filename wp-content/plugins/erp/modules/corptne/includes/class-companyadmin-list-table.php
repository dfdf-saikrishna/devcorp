...
$email =  $item['ADM_Email']; 
    ...
    function process_bulk_action()
    {
        ...
        if ('delete' === $this->current_action()) {
            ...
            if (!empty($ids)) {
                $selcomp = $wpdb->get_results("SELECT user_id FROM $table_name WHERE ADM_Id IN($ids)");
                foreach ($selcomp as $reqcmp){
                $wpdb->query("DELETE FROM $table_name_user WHERE ID IN($reqcmp->user_id)");
                }
                $wpdb->query("UPDATE $table_name SET ADM_Status = 9 WHERE ADM_Id IN($ids)");
    ...
    function prepare_items()
    {
        ...
        $total_items = $wpdb->get_var("SELECT COUNT(COM_Id) FROM $table_name WHERE ADM_Status=1");
		...
		if(!empty($_POST["s"])) {
			...
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name".$query."AND ADM_Status=1 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
		else{
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE ADM_Status=1 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
