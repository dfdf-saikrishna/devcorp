...
//    }
   ...
   $user_id = $data['company']['COM_Id'];
    ...
    if($update){
        $tablename = "company";
        $company_data['COM_Id'] = $user_id;
        $wpdb->update( $tablename,$company_data,array( 'COM_Id' => $user_id ));    
    }
    else{
   ...
   // $company_data['user_id'] = $user_id;
	...
	$newcmpid = $wpdb->insert_id; 
    $folderName	= 'TNE'.rand(1111,9999).sprintf("%04d", $newcmpid);
    $imdir = WPERP_COMPANY_PATH . "/tally-files/$folderName/";
    if (!file_exists($imdir)) {
	wp_mkdir_p($imdir);
    }
    $ftp_data = array('COM_TallyFolder'=>$folderName);
    $wpdb->update( company,$ftp_data,array( 'COM_Id' => $newcmpid )); 
   // return $user_id;
    }
    //return $user_id;
