<?php
   ...
   */
	...
	$action        = '';
	//$currentDir	   = 'http://corptne.com/payment/';
	$currentDir	   =  'http://corptne.com/wp-admin/admin.php?page=';
	$posted = array();
	if(!empty($_POST)) {
...
<html>
  ...
  <body onload="submitPayuForm()">
    ...
    $Tid = $_GET['Tid'];
    ?>
    <h2>Please Wait while we transfer you to Payment Gateway</h2>
    <div id="hide_payment">
    <h2>PayU Form</h2>
    <br/>
    <?php if($formError) { ?>
    ...
    <form action="<?php echo $action; ?>" method="post" name="payuForm">
      ...
      <table>
        ...
        <tr>
          ...
          <td><input name="amount" type="number" value="<?php echo (empty($total)) ? '' : $total ?>" /></td>
          <td>First Name <span class="mand">*</span>: </td>
          <td><input type="text" name="firstname" id="firstname" value="<?php echo (empty($_GET['emp_namepp'])) ? '' : $_GET['emp_namepp']; ?>" /></td>
        </tr>
        <tr>
          <td>Email <span class="mand">*</span>: </td>
          <td><input type="email" name="email" id="email" value="<?php echo (empty($_GET['emp_email'])) ? '' : $_GET['emp_email']; ?>" /></td>
          <td>Phone <span class="mand">*</span>: </td>
          <td><input type="text" name="phone" value="<?php echo (empty($_GET['phone'])) ? '' : $_GET['phone']; ?>" /></td>
        </tr>
        <tr>
          <td>Product Info <span class="mand">*</span>: </td>
          <td colspan="3"><textarea name="productinfo"><?php echo "Flight Ticket Booking"; ?></textarea></td>
        </tr>
        <tr>
          <td>Success URL <span class="mand">*</span>: </td>
          <td colspan="3"><input type="text" name="surl" value="<?php echo (empty($posted['surl'])) ? $currentDir.'success-flight.php&Tid='.$Tid : $posted['surl'] ?>" size="64" /></td>
        </tr>
        <tr>
          <td>Failure URL <span class="mand">*</span>: </td>
          <td colspan="3"><input type="text" name="furl" value="<?php echo (empty($posted['furl'])) ? $currentDir.'failure-flight.php' : $posted['furl'] ?>" size="64" /></td>
        </tr>
        ...
        <tr>
          <?php if(!$hash) { ?>
            <td colspan="4"><input type="submit" id="payment" value="Submit" /></td>
          <?php } ?>
        </tr>
      </table>
    </form>
