        ...
        <div class="wrap pre-travel-request erp" id="wp-erp">
            ...
            <table class="table" border="0" id="table-mileage-travel">
				...
				?>
                      ...
                      <td data-title="Date" class="scrollmsg"><input name="txtDate[]" id="txtDate<?php echo $rows; ?>" class="posttraveldate form-control SmallInput" placeholder="dd-mm-yyyy" autocomplete="off" value="<?php if($rowrequest->RD_Dateoftravel=="0000-00-00 00:00:00") echo ""; else echo date('d-m-Y',strtotime($rowrequest->RD_Dateoftravel)); ?>"/><input name="txtStartDate[]" id="txtStartDate1" class="form-control SmallInput" placeholder="dd/mm/yyyy" autocomplete="off" style="display:none;" value="n/a" />
                        <input name="txtEndDate[]" id="txtEndDate<?php echo $rows; ?>" class="form-control SmallInput" placeholder="dd/mm/yyyy" autocomplete="off" style="display:none;" value="n/a" />
                        <input type="text" name="textBillNo[]" id="textBillNo<?php echo $rows; ?>" autocomplete="off"  class="form-control SmallInput" style="display:none;" value="n/a"/></td>
                      <td data-title="Description"><textarea name="txtaExpdesc[]" id="txtaExpdesc<?php echo $rows; ?>" class="form-control SmallInput" autocomplete="off"><?php echo stripslashes($rowrequest->RD_Description) ?></textarea>
                        <select name="selExpcat[]" id="selExpcat<?php echo $rows; ?>" class="form-control SmallInput" style="display:none;">
                          <option value="5">select</option>
                        </select></td>
                       <td data-title="Category"><span id="modeoftr<?php echo $rows; ?>acontent">
                        <select name="selModeofTransp[]" id="selModeofTransp<?php echo $rows; ?>" class="form-control SmallInput" style="width:110px;" onchange="return getAmount(this.value, <?php echo $rows; ?>);">
                      ...
                      <td data-title="City/Location"><span id="city<?php echo $rows; ?>container">
                        <input  name="from[]" id="from<?php echo $rows; ?>" type="text" placeholder="From" class="form-control SmallInput" value="<?php echo $rowrequest->RD_Cityfrom?>" autocomplete="off">
                        <input  name="to[]" id="to<?php echo $rows; ?>" type="text" placeholder="To" class="form-control SmallInput" value="<?php echo $rowrequest->RD_Cityto ?>" autocomplete="off">
                        </span><select name="selStayDur[]" class="form-control SmallInput" style="display:none;">
                          <option value="n/a">Select</option>
                        </select></td>
                      <td data-title="Distance (in km)"><input type="text" class="form-control SmallInput" name="txtdist[]" id="txtdist<?php echo $rows; ?>" value="<?php echo $rowrequest->RD_Distance ?>" onkeyup="return mileageAmount(this.value, <?php echo $rows; ?>);" autocomplete="off"/></td>
                      <td data-title="Total Cost"> <input type="text" class="form-control SmallInput" name="txtCost[]" id="txtCost<?php echo $rows; ?>" value="<?php echo $rowrequest->RD_Cost;?>" readonly="readonly"  autocomplete="off"/></td>
					...
					?>
                        ...
                        <?php $j++; } ?>
                        <input type='file' name='file<?php echo $rows; ?>[]' id="file<?php echo $rows; ?>[]" multiple="true" onchange="Validate(this.id);" style="width:150px;"><input type="hidden" value="<?php echo $rowrequest->RD_Id; ?>" name="rdids[]"/></td>
                        <td><button type="button" value="<?php echo $rowrequest->RD_Id; ?>" class="btn btn-default" name="deleteRowbutton" onclick="return checkDeletRow();" id="deleteRowbutton" title="delete row" <?php if($cnt==1) echo 'disabled="disabled"'; ?> ><i class="fa fa-trash-o"></i></button></td>
                    </tr>
                    <?php 
                ...
                <div class="hidden-sm hidden-md hidden-xs" style="float:right;"><a title="Add Rows" class="btn btn-success btn-sm"><span id="add-row-mileage-edit">Add +</span></a> <span class="removebuttoncontainer"></span></div>
                <div class="hidden-lg" style="float:left;  padding-top:10px; padding-bottom:10px;"><a title="Add Rows" class="btn btn-success btn-sm"><span id="add-row-mileage-edit">Add +</span></a> <span class="removebuttoncontainer"></span></div>
				<span id="totaltablepost"> </span>
            </div>
            <div id="my_centered_buttons">
            <span class="erp-loader" style="margin-left:67px;margin-top: 4px;display:none"></span>
            <input type="submit" name="update-pre-travel-request" id="update-pre-travel-request" value="Update" class="btn btn-success btn-md">
           
            <button type="button" name="reset" id="reset" class="btn btn-warning">Reset</button>
            </div>
            </form>
			</div>
            <div style="margin-top:10px" id="grade-limit" class="postbox leads-actions closed">
