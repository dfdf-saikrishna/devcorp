    ...
    function extra_tablenav($which) {
        ...
        function get_sortable_columns() {
            $sortable_columns = array(
                ...
                'rep_manager_code' => array('Reporting Manager', true),
                'skiplevel_manager_approval' => array('SkipLevel Manager', true),
                'finance_approval' => array('Finance', true),
        ...
        function prepare_items() {
            ...
            if (!empty($_POST["s"])) {
                ...
                $total_items = count($wpdb->get_results("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM requests req, request_employee re " . $query . "AND req.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id != $mydetails->EMP_Id AND  req.REQ_Active !=9 AND re.RE_Status=1"));
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM requests req, request_employee re " . $query . "AND req.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id != $mydetails->EMP_Id  AND req.REQ_Active !=9 AND re.RE_Status=1 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            } else {
                $total_items = count($wpdb->get_results("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM requests req, request_employee re WHERE req.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id != $mydetails->EMP_Id  AND req.REQ_Active !=9 AND re.RE_Status=1 " . $query));

                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM requests req, request_employee re WHERE req.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id != $mydetails->EMP_Id AND  req.REQ_Active !=9 AND re.RE_Status=1 " . $query . " ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            }
