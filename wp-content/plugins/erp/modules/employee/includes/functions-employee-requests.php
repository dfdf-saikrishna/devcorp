            ...
            case 3:
                ...
                if ($expenseLimit > 0) {
                    if ($mydetails->EMP_Code == $mydetails->EMP_Funcrepmngrcode) {
                        ...
                        // insert into request
                        $wpdb->insert('requests', array('REQ_Status' => 2, 'POL_Id' => 5, 'REQ_Code' => $expreqcode, 'COM_Id' => $compid, 'RT_Id' => $etype, 'PC_Id' => $selProjectCode, 'costCenter_Id' => $selCostCenter));
                        $reqid = $wpdb->insert_id;
...
//                        }
        ...
        if ($etype == 2) {
            header("location:/wp-admin/admin.php?page=Post-travel&status=success&msg=You have successfully added a Post Travel Expense Request  <br> Your Request Code: $expreqcode <br> Please wait for approval..");
            
            exit;
        } else if ($etype == 5) {
            header("Location: /wp-admin/admin.php?page=create-mileage&status=success&msg=You have successfully added a Mileage Expense Request  <br> Your Request Code: $expreqcode <br> Please wait for approval..");
            exit;
        } else if ($etype == 6) {
            header("Location: /wp-admin/admin.php?page=create-others&status=success&msg=You have successfully added a Utility Request  <br> Your Request Code: $expreqcode <br> Please wait for approval..");
            exit;
        } else if ($etype == 3) {
            header("Location: /wp-admin/admin.php?page=create-others&status=success&msg=You have successfully added a General Expense Request  <br> Your Request Code: $expreqcode <br> Please wait for approval..");
            exit;
        }
    ...
    if (isset($_POST['bookTickets'])) {
		...
		if($status = $wpdb->get_row("SELECT * FROM save_status WHERE REQ_Id = '$reqid'")){
			$msg="12";
			header("location:/wp-admin/admin.php?page=View-Post-Request&reqid=$reqid&msg=$msg");
			die;
		}
        ...
        header("location:/wp-admin/admin.php?page=View-Post-Request&reqid=$reqid&msg=$msg&a=$a&b=$b&d=$d&m=$m&c=$c&s=$s&p=$p&h=$h&f=$f");
        exit;
    }
    if (isset($_POST['buttonSelfbooking'])) {
        ...
        header("location:/wp-admin/admin.php?page=View-Request&reqid=$reqid&msg=$msg&s=$s&a=$a&c=$c&b=$b&d=$d&p=$p&h=$h&f=$f");
        exit;
    }
    if (isset($_POST['cancelTickets'])) {
        ...
        header("location:/wp-admin/admin.php?page=View-Post-Request&reqid=$reqid&msg=$msg&h=$h&p=$p&f=$f&b=$b&c=$c&s=$s&a=$a&d=$d");
        exit;
    }
    if (isset($_POST['submitUpdPretrvReq'])) {
        global $wpdb;
        $compid = $_SESSION['compid'];
        $posted = array_map('strip_tags_deep', $_POST);
        ...
        $ptcid = $wpdb->insert("pre_travel_claim", array('REQ_Id' => $reqid));
        if ($ins) {
        ...
        if (count($txtAcualCost) == "" && $reqid == "") {
            //$response = array('status' => 'success', 'message' => "Some fields went missing.");
            //$this->send_success($response);
            //exit;
...
//            $this->send_success($response);
            ...
            //$wpdb->update('pre_travel_claim', "PTC_Status=1, PTC_RepMngrEmpid='$repmngrid', PTC_RepMngrStatus='$repmngrstatus', PTC_RepMngrApprovedDate=$repmngrapprvrdate, PTC_RepMngrRejectedDate=NULL, PTC_FinanceEmpid=0, PTC_FinanceStatus=1, PTC_FinanceApprovedDate=NULL, PTC_FinanceRejectedDate=NULL", "REQ_Id='$reqid'", $filename);
            ...
            //exit;
           // header("location:/wp-admin/admin.php?page=View-Request&reqid=$reqid&msg=$msg&h=$h&p=$p&f=$f&b=$b&c=$c&s=$s&a=$a&d=$d");
$msg="claim submitted succesfully";
            header("location:/wp-admin/admin.php?page=Submit+Claim&msg=$msg&reqid=$reqid");
           exit;
        }
    }
}
