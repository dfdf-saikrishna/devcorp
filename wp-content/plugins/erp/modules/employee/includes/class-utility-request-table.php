   ...
   $claimdata = '<span class="status-2 title="Claimed on: ' . date("d/M/y", strtotime($item['REQ_ClaimDate'])) . '">Claimed</span>';
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'reporting_manager_approval' => __('Reporting Manager', 'companiesadmin_table_list'),
            'skiplevel_manager_approval' => __('SkipLevel Manager', 'emp_req_table_list'),
            'finance_approval' => __('Finance', 'companiesadmin_table_list'),
    ...
    function get_sortable_columns()
    {
        $sortable_columns = array(
            ...
            'reporting_manager_approval' => array('Reporting Manager', false),
            'skiplevel_manager_approval' => array('SkipLevel Manager', true),
            'finance_approval' => array('Finance', false),
    ...
    function prepare_items()
    {
        ...
        // notice that last argument is ARRAY_A, so we will retrieve array
			...
			foreach( $searchcol as $col) {
				...
				if(!empty($_REQUEST["s"])) {$query .=  ' '.$col.' LIKE "'.$search.'"';}
				$i++;
			}
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM requests req, request_employee re, policy pol WHERE ".$query." AND req.COM_Id='$compid' AND req.RT_Id=6 AND re.EMP_Id='$empuserid' AND req.POL_Id=pol.POL_Id AND req.REQ_Id=re.REQ_Id AND req.REQ_Active=1 AND re.RE_Status=1 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
		else{
