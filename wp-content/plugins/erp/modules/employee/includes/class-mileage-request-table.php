    ...
    function prepare_items()
    {
        ...
        // notice that last argument is ARRAY_A, so we will retrieve array
			...
			foreach( $searchcol as $col) {
				...
				if(!empty($_REQUEST["sm"])) {$query .=  ' '.$col.' LIKE "'.$search.'"';}
				$i++;
			}
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM requests req, request_employee re WHERE ".$query." AND req.COM_Id='$compid' AND req.RT_Id=5 AND re.EMP_Id='$empuserid' AND req.REQ_Id=re.REQ_Id AND req.REQ_Active=1 AND re.RE_Status=1 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
		else{
