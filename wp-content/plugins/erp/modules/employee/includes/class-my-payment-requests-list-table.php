 ...
 */
class My_Payment_Requests_List extends \WP_List_Table {
        ...
        */
        ...
        function prepare_items() {
            ...
            if (!empty($_POST["s"])) {
                ...
                $total_items = count($wpdb->get_results("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM employees emp, requests req, request_employee re, request_details rd, request_status rs" . $query . "AND re.EMP_Id='$empuserid' AND rd.REQ_Id = req.REQ_Id AND rd.RD_Dateoftravel <= CURDATE() AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND req.REQ_Active !=9 AND re.RE_Status=1 AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.REQ_Id = rs.REQ_Id AND rs.REQ_Status = '2' AND rs.RS_EmpType = '1'"));
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM employees emp, requests req, request_employee re, request_details rd, request_status rs" . $query . "AND re.EMP_Id='$empuserid' AND rd.REQ_Id = req.REQ_Id AND rd.RD_Dateoftravel <= CURDATE() AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND req.REQ_Active !=9 AND re.RE_Status=1 AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.REQ_Id = rs.REQ_Id AND rs.REQ_Status = '2' AND rs.RS_EmpType = '1' ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            } else {
                $total_items = count($wpdb->get_results("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM employees emp, requests req, request_employee re, request_details rd, request_status rs WHERE re.EMP_Id='$empuserid' AND req.REQ_Id=re.REQ_Id AND rd.REQ_Id = req.REQ_Id AND rd.RD_Dateoftravel <= CURDATE() AND emp.EMP_Id=re.EMP_Id AND req.REQ_Active !=9 AND re.RE_Status=1 AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.REQ_Id = rs.REQ_Id AND rs.REQ_Status = '2' AND rs.RS_EmpType = '1' " . $query));

                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM employees emp, requests req, request_employee re, request_details rd, request_status rs WHERE re.EMP_Id='$empuserid' AND req.REQ_Id=re.REQ_Id AND rd.REQ_Id = req.REQ_Id AND rd.RD_Dateoftravel <= CURDATE() AND emp.EMP_Id=re.EMP_Id AND req.REQ_Active !=9 AND re.RE_Status=1 AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.REQ_Id = rs.REQ_Id AND rs.REQ_Status = '2' AND rs.RS_EmpType = '1'" . $query . " ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            }
