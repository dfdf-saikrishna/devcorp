			...
			currntModVal=parseInt(currntModVal);
				...
				if(ModLimitVal_0!=0){
					...
					if(currntModVal==5 || currntModVal==6){
						...
						var stayDur		=	document.getElementById('stayDays'+row);

						var stayDurNos          =	parseInt(stayDur.innerText);
					...
					//console.log(chks[i].value);
					//console.log(costcont +'>'+ModLimitVal_0);
					percentcal = ModLimitVal_0;
					//console.log(stayDurNos);
					if(hotel)
					...
					if(costcont > percentcal){
						...
						$('.exceed-hotel'+row).addClass('ired');
						document.getElementById("expenseLimit").value = "1";
	                    document.getElementById('show-exceed-hotel'+row).innerHTML="Your "+ModLimitVal_1+" expense limit is upto "+ModLimitVal_0+" on per day basis.";
						}
						else{               
	                                        //document.getElementById('show-exceed'+row).innerHTML="Your "+ModLimitVal_1+" expense limit is upto "+ModLimitVal_0;                  
	                                        ...
	                                        if(mode==1)
	                                        {
	                                        $('.exceed-flight'+row).addClass('ired');
	                                        document.getElementById('show-exceed-flight'+row).innerHTML="Your "+ModLimitVal_1+" expense limit is upto "+ModLimitVal_0;
	                                        }
                                            if(mode==2) 
                                            {
                                            $('.exceed-bus'+row).addClass('ired');
                                            document.getElementById('show-exceed-bus'+row).innerHTML="Your "+ModLimitVal_1+" expense limit is upto "+ModLimitVal_0;
	                                        }
											if(mode==3) 
                                            {
                                            $('.exceed-car'+row).addClass('ired');
                                            document.getElementById('show-exceed-car'+row).innerHTML="Your "+ModLimitVal_1+" expense limit is upto "+ModLimitVal_0;
	                                        }
	                                        if(mode==4) 
                                            {
                                            $('.exceed-others'+row).addClass('ired');
                                            document.getElementById('show-exceed-others'+row).innerHTML="Your "+ModLimitVal_1+" expense limit is upto "+ModLimitVal_0;
	                                        }
											if(mode==5) 
											{
											$('.exceed-hotel'+row).removeClass('ired');
											document.getElementById('show-exceed-hotel'+row).innerHTML="";
											}
	                                        }
	                    ...
	                    if(mode==1)
                        {
                        $('.exceed-flight'+row).removeClass('ired');
                        document.getElementById('show-exceed-flight'+row).innerHTML="";
                        }
                        if(mode==2) 
                        {
                        $('.exceed-bus'+row).removeClass('ired');
                        document.getElementById('show-exceed-bus'+row).innerHTML="";
                        }
						if(mode==3) 
						{
						$('.exceed-car'+row).removeClass('ired');
						document.getElementById('show-exceed-car'+row).innerHTML="";
						}
                        if(mode==4) 
                        {
                        $('.exceed-others'+row).removeClass('ired');
                        document.getElementById('show-exceed-others'+row).innerHTML="";
                        }
						if(mode==5) 
                        {
                        $('.exceed-hotel'+row).removeClass('ired');
...
function getTotal(hotel,currntDur)
{
	...
	var chks=document.getElementsByName('other_cost[]');
	if(chks.length>0){
	costotint=parseInt(chks[0].value.trim());
	if(costotint){
	totalcost+=costotint;
	}
	}
...
function getTotalPost(hotel,currntDur)
{
	var chks=document.getElementsByName('txtCost[]');
	//alert(chks);
	var totalcost=0;
	...
	if(totalcost!=0 && totalcost!=""){
		...
		document.getElementById('totaltablepost').innerHTML='<table class="wp-list-table widefat striped admins" style="font-weight:bold;"><tr ><td align="right" width="85%">Claim Amount</td><td align="center" width="5%">:</td><td align="right" width="10%">'+totalcost+'.00</td></tr></table>';
		//document.getElementById('totaltable').value = totalcost;
...
function getMotPosttravel(n,rownumber)
{
	...
	if(n==1)
	{		
		content='<select name="selModeofTransp[]" style="width: 100px!important;"  class="selModeofTransp form-control" id="selModeofTransp'+rownumber+'" onchange="setFromTo(this.value, '+rownumber+');"><option value="">Select</option><?php $selsql=$wpdb->get_results("SELECT MOD_Id, MOD_Name FROM mode WHERE EC_Id=1 AND COM_Id IN (0, '$compid') AND MOD_Status=1"); foreach($selsql as $rowsql){ ?><option value="<?php echo $rowsql->MOD_Id; ?>"><?php echo $rowsql->MOD_Name; ?></option><?php } ?></select>';
		
		
		
		citycontent='<input  name="from[]" id="'+selfromIt+'" type="text" placeholder="From" class="form-control" autocomplete="off"><input  name="to[]" id="'+seltoIt+'" type="text" placeholder="To" class="form-control" autocomplete="off"><select name="selStayDur[]" class="form-control" id="'+stayDur+'" style="display:none;"><option value="n/a">Select</option></select>';		
	...
	else if(n==2)
	{
		...
		content='<select name="selModeofTransp[]" class="form-control" style="width: 100px!important;" onchange="chkCostPost(this.value, '+rownumber+');"><option value="">Select</option><?php $selsql=$wpdb->get_results("SELECT MOD_Id, MOD_Name FROM mode WHERE EC_Id=2 AND COM_Id IN (0, '$compid') AND MOD_Status=1"); foreach($selsql as $rowsql){ ?><option value="<?php echo $rowsql->MOD_Id; ?>"><?php echo $rowsql->MOD_Name; ?></option><?php } ?></select>';		
		
		citycontent='<input  name="from[]" class="form-control" id="'+selfromIt+'" type="text" placeholder="Location" autocomplete="off"><input  name="to[]" id="'+seltoIt+'" type="text" class="form-control" placeholder="To" value="n/a" style="display:none;"><select name="selStayDur[]" id="'+stayDur+'" class="form-control" onchange="chkNDPost(this.value, '+rownumber+');"><option value="">Select</option><?php $selsql=$wpdb->get_results("SELECT SD_Id, SD_Name FROM stay_duration"); foreach($selsql as $rowsql){?><option value="<?php echo $rowsql->SD_Id;?>"><?php echo $rowsql->SD_Name;?></option><?php } ?></select>';
	...
	{
		content='<select name="selModeofTransp[]" class="form-control" style="width: 100px!important;"><option value="">Select</option><?php $selsql=$wpdb->get_results("SELECT MOD_Id, MOD_Name FROM mode WHERE EC_Id=4 AND COM_Id IN (0, '$compid') AND MOD_Status=1"); foreach($selsql as $rowsql){ ?><option value="<?php echo $rowsql->MOD_Id; ?>"><?php echo $rowsql->MOD_Name; ?></option><?php } ?></select>';
		
		
		citycontent='<input  name="from[]" id="'+selfromIt+'" class="form-control" type="text" placeholder="Location" autocomplete="off"><input  name="to[]" id="'+seltoIt+'" type="text" class="form-control" style="display:none;" placeholder="To" value="n/a"><select name="selStayDur[]" class="form-control" id="'+stayDur+'" style="display:none;"><option value="n/a">Select</option></select>';	
