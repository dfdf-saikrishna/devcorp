...
class Request_Travel_Expenses extends \WP_List_Table {
    ...
    function column_estimated_cost($item) {
		...
		$totalcost = $wpdb->get_row("SELECT SUM(RD_Cost) AS total FROM request_details WHERE REQ_Id=$item[REQ_Id] AND RD_Status=1");
                ...
                $selptc = $wpdb->get_row("SELECT * FROM pre_travel_claim WHERE REQ_Id='$item[REQ_Id]'");
               
                $status1 = ( isset($selptc->PTC_RepMngrStatus) ) ? $selptc->PTC_RepMngrStatus : '';
//echo $selEmployee;
                $status = ( isset($selptc->PTC_FinanceStatus) ) ? $selptc->PTC_FinanceStatus : '';
        ...
        if ($item['REQ_Type'] == 4) {
            ...
            if ($item['REQ_Type'] == 3 || $item['REQ_Type'] == 4) {
                ...
                if ($item['REQ_PreToPostStatus']) {
                    ...
                    $href = "/wp-admin/admin.php?page=View-Post-Request&selExpenseType=2&reqid=" . $item['REQ_Id'];
                } else {
                    switch ($item['RT_Id']) {
                        case 1:
                            $href = "/wp-admin/admin.php?page=View-Request&selExpenseType=1&reqid=" . $item['REQ_Id'];
                            break;
                        ...
                        case 3:
                            $href = "/wp-admin/admin.php?page=View-other-Request&selExpenseType=3&reqid=" . $item['REQ_Id'];
                            break;

                        case 5:
                            $href = "/wp-admin/admin.php?page=View-Mileage-Request&selExpenseType=5&reqid=" . $item['REQ_Id'];
                            break;

                        case 6:
                            $href = "/wp-admin/admin.php?page=View-Utility-Request&selExpenseType=6&reqid=" . $item['REQ_Id'];
                            break;
                    }//return $item['REQ_Type'];
    ...
    function prepare_items() {
        ...
        if (isset($_REQUEST['selExpenseType']) && $_REQUEST['selExpenseType']) {
            $selExpenseType = $_REQUEST['selExpenseType'];
            if ($selExpenseType == 4) {
