 ...
 */
class Emp_Claims_List extends \WP_List_Table {
        ...
        */
	    ...
	    //DELIGATE
            ...
            if (isset($_REQUEST['filter_status']) && $_REQUEST['filter_status']) {
                ...
                $query.=" AND ptc.PTC_RepMngrStatus=$selReqstatus";
            }
            // filter employee		
            ...
            if (!empty($_POST["s"])) {
                ...
                $total_items = count($wpdb->get_results("SELECT DISTINCT(req.REQ_Id), req.* FROM requests req, request_employee re, employees emp, pre_travel_claim ptc" . $query . "AND emp.COM_Id='$compid' AND emp.EMP_Reprtnmngrcode='$rprcode' AND emp.EMP_Id != '$empID' AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.RE_Status=1 AND req.REQ_Active !=9 AND ptc.REQ_Id = req.REQ_Id"));
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT(req.REQ_Id), req.* FROM requests req, request_employee re, employees emp, pre_travel_claim ptc" . $query . "AND emp.COM_Id='$compid' AND emp.EMP_Reprtnmngrcode='$rprcode' AND emp.EMP_Id != '$empID' AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.RE_Status=1 AND req.REQ_Active !=9 AND ptc.REQ_Id = req.REQ_Id ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            } else {
                $total_items = count($wpdb->get_results("SELECT DISTINCT(req.REQ_Id), req.* FROM requests req, request_employee re, employees emp, pre_travel_claim ptc WHERE emp.COM_Id='$compid' AND emp.EMP_Reprtnmngrcode='$rprcode' AND emp.EMP_Id != '$empID' AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.RE_Status=1 AND req.REQ_Active !=9 AND ptc.REQ_Id = req.REQ_Id " . $query));

                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT(req.REQ_Id), req.* FROM requests req, request_employee re, employees emp, pre_travel_claim ptc WHERE emp.COM_Id='$compid' AND emp.EMP_Reprtnmngrcode='$rprcode' AND emp.EMP_Id != '$empID' AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.RE_Status=1 AND req.REQ_Active !=9 AND ptc.REQ_Id = req.REQ_Id " . $query . " ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            }
