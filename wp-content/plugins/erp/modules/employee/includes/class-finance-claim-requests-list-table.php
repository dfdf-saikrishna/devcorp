 ...
 */
class Finance_Claim_Requests_List extends \WP_List_Table {
    ...
    function extra_tablenav($which) {
        ...
        function column_request_code($item) {
            ...
            //} else {
            if(($item['REQ_Type']==1) && ($item['REQ_PreToPostStatus'])){		
			$href="/wp-admin/admin.php?page=View-Post-Accounts-Request&reqid=".$item['REQ_Id'];
			
		} else if($item['REQ_Type'] > 1 && $item['REQ_Type'] != 3 && $item['REQ_Type'] != 4){
        ...
        function prepare_items() {
            ...
            /*if (isset($_REQUEST['filter_status']) && $_REQUEST['filter_status']) {
                ...
                $query.=" AND ptc.PTC_FinanceStatus=$selReqstatus";
            }*/
            // filter employee		
            ...
            if (!empty($_POST["s"])) {
                ...
                $total_items = count($wpdb->get_results("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM requests req, request_employee re, pre_travel_claim ptc " . $query . "req.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id != $mydetails->EMP_Id AND  req.REQ_Active !=9 AND re.RE_Status=1 AND ptc.REQ_Id = req.REQ_Id"));
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM requests req, request_employee re, pre_travel_claim ptc " . $query . "AND req.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id != $mydetails->EMP_Id AND  req.REQ_Active !=9 AND re.RE_Status=1 AND ptc.REQ_Id = req.REQ_Id ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            } else {
                $total_items = count($wpdb->get_results("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM requests req, request_employee re, pre_travel_claim ptc WHERE req.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id != $mydetails->EMP_Id AND  req.REQ_Active !=9 AND re.RE_Status=1 AND ptc.REQ_Id = req.REQ_Id " . $query));

                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT (req.REQ_Id) AS reqids, req.* FROM requests req, request_employee re, pre_travel_claim ptc WHERE req.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id != $mydetails->EMP_Id AND  req.REQ_Active !=9 AND re.RE_Status=1 AND ptc.REQ_Id = req.REQ_Id " . $query . " ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            }
