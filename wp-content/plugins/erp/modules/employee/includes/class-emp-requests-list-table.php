        ...
        */
        ...
        function get_columns() {
            $columns = array(
                ...
                'rep_manager_code' => __('Reporting Manager', 'emp_req_table_list'),
                'skiplevel_manager_approval' => __('SkipLevel Manager', 'emp_req_table_list'),
                'finance_approval' => __('Finance', 'emp_req_table_list'),
	    ...
	    //DELIGATE
			...
			$query.=" AND emp.EMP_Reprtnmngrcode IN ('$selrow->EMP_Reprtnmngrcode', $empcodes) AND emp.EMP_Code!='$mydetails->EMP_Code'";
			
		} else {
	  	
			$query.=" AND emp.EMP_Reprtnmngrcode='$selrow->EMP_Reprtnmngrcode' AND emp.EMP_Code!='$mydetails->EMP_Code'";
            ...
            if (!empty($_POST["s"])) {
                ...
                $total_items = count($wpdb->get_results("SELECT DISTINCT(req.REQ_Id), req.* FROM $table_name req, request_employee re, employees emp  WHERE emp.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.RE_Status=1 AND req.REQ_Active !=9 AND emp.EMP_Code!='$mydetails->EMP_Code'"));
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT(req.REQ_Id), req.* FROM $table_name req, request_employee re, employees emp  WHERE emp.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.RE_Status=1 AND req.REQ_Active !=9 AND emp.EMP_Code!='$mydetails->EMP_Code' ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            } else {
                $total_items = count($wpdb->get_results("SELECT DISTINCT(req.REQ_Id), req.* FROM $table_name req, request_employee re, employees emp  WHERE emp.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.RE_Status=1 AND req.REQ_Active !=9 AND emp.EMP_Code!='$mydetails->EMP_Code' " . $query));

                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT(req.REQ_Id), req.* FROM $table_name req, request_employee re, employees emp  WHERE emp.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id AND emp.EMP_Status=1 AND emp.EMP_Access=1 AND re.RE_Status=1 AND req.REQ_Active !=9 AND emp.EMP_Code!='$mydetails->EMP_Code' " . $query . " ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            }
