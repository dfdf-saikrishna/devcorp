 ...
 */
class Others_Requests_List extends \WP_List_Table
{
    /**
   ...
   $claimdata = '<span class="status-2 title="Claimed on: ' . date("d/M/y", strtotime($item['REQ_ClaimDate'])) . '">Claimed</span>';
    ...
    function prepare_items()
    {
        ...
        // notice that last argument is ARRAY_A, so we will retrieve array
			...
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM requests req, request_employee re, policy pol WHERE ".$query." AND req.COM_Id='$compid' AND req.RT_Id=3 AND re.EMP_Id='$empuserid' AND req.POL_Id=pol.POL_Id AND req.REQ_Id=re.REQ_Id AND req.REQ_Active=1 AND re.RE_Status=1 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
		else{
