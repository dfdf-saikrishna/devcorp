 ...
 */
class TravelDesk_Claims extends \WP_List_Table {
...
//            $amnt+=$item['TDC_ServiceCharges'];
            return IND_money_format($item['TDC_Amount']) . ".00";
        ...
        function column_invoiceref($item) {
            //$href = "";
            return "<a href= '/wp-admin/admin.php?page=TravelClaimsView&tdcid=$item[TDC_Id]' >" . $item['TDC_ReferenceNo'] . "</a>";
        ...
        function prepare_items() {
           ...
           // $table_name = 'travel_desk_claims'; // do not forget about tables prefix
            ...
            if (!empty($_POST["s"])) {
                ...
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM travel_desk_claims " . $query . " AND  COM_Id='$compid' ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            } else {
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM travel_desk_claims WHERE  COM_Id='$compid' " . $query . " ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            }
