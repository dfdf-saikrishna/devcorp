...
class Travel_Agent_User_List_Table extends \WP_List_Table
{
    ...
    function process_bulk_action()
    {
        ...
        if (!empty($ids)) {
        		...
        		if($user_id = $wpdb->get_row("SELECT user_id FROM superadmin WHERE SUP_Id IN($ids) AND SUP_Status=1 AND SUP_Refid='$supid'")){
        		$wpdb->query("UPDATE superadmin SET SUP_Status=9 WHERE SUP_Id IN($ids) AND SUP_Status=1 AND SUP_Refid='$supid'");
        		$wpdb->query("DELETE FROM wp_users WHERE ID IN($user_id->user_id)");
