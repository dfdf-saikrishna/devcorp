...
class Booking_Requests_List_Table extends \WP_List_Table
{
    ...
    function prepare_items()
    {
        ...
        $total_items = count($wpdb->get_results("SELECT * FROM requests WHERE COM_Id='$compid' AND REQ_Type = 2 AND REQ_Active != 9"));
       // $total_items=count($total1_items);
		...
		if(!empty($_POST["s"])) {
			...
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name ".$query." AND COM_Id='$compid' AND REQ_Type = 2 AND REQ_Active != 9 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
		else{
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE COM_Id='$compid' AND REQ_Type = 2 AND REQ_Active != 9 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
