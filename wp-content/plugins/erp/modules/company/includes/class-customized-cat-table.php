    ...
    function column_mode($item) {
        ...
        if (count($selexpcat)) {
            ...
            foreach ($selexpcat as $rowexpcat)
                array_push($modes, $rowexpcat->MOD_Id);
            ...
            foreach ($modes as $mo) {
            	...
            	$modeName = $wpdb->get_row("SELECT MOD_Name FROM mode WHERE MOD_Id=$mo AND COM_Id ='$compid'");
            	
                $ol.='<li type="none">' . $k . ". " . $modeName->MOD_Name . '</li> <div class="row-actions"><span class="edit"><a href="?page=projects" class="subcats" data-id="'.$mo.'">Edit</a> | </span><span class="delete"><a href="?page=addcat&amp;action=delete&amp;id='.$mo.'">Delete</a></span></div>';
      ...
      function column_expense($item) {
        $actions = array(
            ...
            'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['EC_Id'], __('Delete', 'subcat-table-list')),
             ...
             return sprintf('%s',$item['EC_Name'], $this->row_actions($actions)
//        return sprintf('%s %s',
...
//            $modes=$ol."</ol>";
    ...
    function get_columns() {
        $columns = array(
            ...
            'modes' => __('Modes', 'subcat-table-list'),
            'mode' => __('SubCategory', 'subcat-table-list'),
        );
        return $columns;
    }
