    ...
    function column_name($item) {
        ...
        if ($cnt = count($wpdb->get_results("SELECT PC_Id FROM project_code WHERE PC_Id='$item[PC_Id]' AND COM_Id='$compid' AND PC_Active = 1"))) {
            ...
            if ($item['PC_Status'] == 1) {
				...
				$actions = array(
				'edit' => sprintf('<a href="?page=projects" data-id=%s">%s</a>', $item['PC_Id'], __('Edit', 'costcenter_table_list')),
        ...
        return sprintf('%s %s', '<strong>' . $item['PC_Code'] . '</strong>', 
...
//        die;
    ...
    function get_columns() {
        $columns = array(
            ...
            'Branch' => __('Company Branch', 'project_table_list'),
        ...
        if ('delete' === $this->current_action()) {
            ...
            if (!empty($ids)) {
                ...
                if ($cnt = count($wpdb->get_results("SELECT PC_Id FROM project_code WHERE PC_Id='$pcid' AND COM_Id='$compid' AND PC_Active = 1"))) {
                    ...
                    if ($cnt > 0) {
                        $wpdb->query("UPDATE project_code SET PC_Status=2 ,PC_ClosedDate=NOW() WHERE PC_Status=1 AND PC_Active=1 AND PC_Id IN($pcid)");
		...
		if ('retrieve' === $this->current_action()) {
            ...
            if (!empty($ids)) {
                ...
                if ($cnt = count($wpdb->get_results("SELECT PC_Id FROM project_code WHERE PC_Id='$pcid' AND COM_Id='$compid' AND PC_Active = 1"))) {
                    ...
                    if ($cnt > 0) {
                        $wpdb->query("UPDATE project_code SET PC_Status=1 ,PC_ClosedDate=Null ,PC_AddedDate=NOW() WHERE PC_Status=2 AND PC_Active=1 AND PC_Id IN($pcid)");
    ...
    function prepare_items() {
        ...
        $total_items = count($wpdb->get_results("SELECT * FROM  project_code WHERE CC_Id!='' AND Costcenter_Branch!='' AND COM_Id='$compid' AND PC_Active=1"));

        $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM  project_code WHERE CC_Id!='' AND Costcenter_Branch!='' AND COM_Id='$compid' AND PC_Active=1 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
        //print_r($test);die;
