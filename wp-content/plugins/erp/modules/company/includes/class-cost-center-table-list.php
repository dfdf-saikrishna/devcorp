...
class CostCenter_List_Table extends \WP_List_Table {
    ...
    function column_name($item) {
        ...
        if ($cnt = count($wpdb->get_results("SELECT CC_Id FROM cost_center WHERE CC_Id='$item[CC_Id]' AND COM_Id='$compid' AND CC_Active = 1"))) {
            ...
            if ($item['CC_Status'] == 1) {
				...
				$actions = array(
					...
					'delete' => sprintf('<a href="?page=%s&action=retrieve&id=%s">%s</a>', $_REQUEST['page'], $item['CC_Id'], __('Retrieve', 'costcenter_table_list')),
				);
        ...
        $actions = array(
            'edit' => sprintf('<a href="?page=projects" data-id=%s">%s</a>', $item['CC_Id'], __('Edit', 'costcenter_table_list')),
            //'delete' => sprintf($delete),
            'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['CC_Id'], __('Close', 'costcenter_table_list')),
        );
        return sprintf('%s %s', '<strong>' . $item['CC_Code'] . '</strong>', $this->row_actions($actions)
                // return sprintf('%s %s', $item['MOD_Name'], $this->row_actions($actions)
        );
    }
    /*
...
//        die;
    ...
    function get_columns() {
        $columns = array(
            ...
            'budget' => __('CC Budget', 'costcenter_table_list'),
        ...
        if ('delete' === $this->current_action()) {
            ...
            if (!empty($ids)) {
                ...
                if ($cnt = count($wpdb->get_results("SELECT CC_Id FROM cost_center WHERE CC_Id='$ccid' AND COM_Id='$compid' AND CC_Active = 1"))) {
                    ...
                    if ($cnt > 0) {
                        $projects = $wpdb->get_results("SELECT PC_Id FROM project_code WHERE CC_Id='$ccid' AND COM_Id='$compid' AND PC_Active = 1");
                        foreach ($projects as $value){
    			$wpdb->query("UPDATE project_code SET PC_Active=0 WHERE PC_Status=1 AND PC_Active=1 AND PC_Id IN($value->PC_Id)");
    			}
                        $wpdb->query("UPDATE cost_center SET CC_Status=2 ,CC_ClosedDate=NOW() WHERE CC_Status=1 AND CC_Active=1 AND CC_Id IN($ccid)");
            ...
            if (!empty($ids)) {
                ...
                if ($cnt = count($wpdb->get_results("SELECT CC_Id FROM cost_center WHERE CC_Id='$ccid' AND COM_Id='$compid' AND CC_Active = 1"))) {
                    ...
                    if ($cnt > 0) {
                        $projects = $wpdb->get_results("SELECT PC_Id FROM project_code WHERE CC_Id='$ccid' AND COM_Id='$compid' AND PC_Active = 1");
                        foreach ($projects as $value){
    			$wpdb->query("UPDATE project_code SET PC_Active=1 WHERE PC_Status=1 AND PC_Active=0 AND PC_Id IN($value->PC_Id)");
    			}
                        $wpdb->query("UPDATE cost_center SET CC_Status=1 ,CC_ClosedDate=Null ,CC_AddedDate=NOW() WHERE CC_Status=2 AND CC_Active=1 AND CC_Id IN($ccid)");
