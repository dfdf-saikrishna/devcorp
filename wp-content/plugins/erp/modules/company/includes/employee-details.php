...
$et = 1;
$row = $wpdb->get_row("SELECT * FROM requests WHERE COM_Id='$compid' AND REQ_Id='$reqid' AND RT_Id IN (1,2,3,4,5,6) AND REQ_Active != 9");
...
<?php } else { ?>
    ...
    $selexpcat = $wpdb->get_row("SELECT * FROM project_code WHERE COM_Id='$compid' AND PC_Active=1 $pc");
    ...
    if (count($selexpcat) && $showProCode) {
        ...
        if ($row->PC_Id) {
            ...
            if ($rowpcname = $wpdb->get_row("SELECT PC_Code,PC_Name FROM project_code COM_Id='$compid' AND PC_Id=$row->PC_Id AND PC_Active=1")) {
    ...
    <?php } else { ?>
            ...
            <?php } ?>
            <?PHP
            $selexpcat = $wpdb->get_row("SELECT * FROM cost_center WHERE COM_Id='$compid' AND CC_Active=1 $cc");
        ...
        if ($row->CC_Id) {
            ...
            if ($rowpcname = $wpdb->get_row("SELECT CC_Code, CC_Name  FROMcost_center WHERE COM_Id='$compid' AND CC_Id=$row[CC_Id] AND CC_Active=1")) {
