...
/**
 * [single tab personal & family info description]
 *
 * @return void
 */
function erp_company_employee_single_tab_personal($employee) {
   
    include WPERP_COMPANY_VIEWS . '/employee/Personal-Family.php';
}

// bank acoount details
function erp_company_employee_single_tab_bank($employee) {
    include WPERP_COMPANY_VIEWS . '/employee/BankDetails.php';
}

function erp_company_employee_single_tab_driving($employee) {
    include WPERP_COMPANY_VIEWS . '/employee/Driving.php';
}

function erp_company_employee_single_tab_visapassport($employee) {
    include WPERP_COMPANY_VIEWS . '/employee/VisaPassport.php';
}

function erp_company_employee_single_tab_frequent($employee) {
    include WPERP_COMPANY_VIEWS . '/employee/Frequent.php';
}

function erp_company_employee_single_tab_medical($employee) {
    include WPERP_COMPANY_VIEWS . '/employee/MedicalInfo.php';
}
...
function companyemployee_create($args = array()) {
    ...
    $user_id_update = isset($data['companyemployee']['user_id']) ? intval($data['companyemployee']['user_id']) : 0;
    
    if ($user_id) {
        $userdata['ID'] = $user_id_update;
    } else {
    ...
    if ($user_id_update) {
        $tablename = "employees";
        $companyemployee_data['user_id'] = $user_id_update;
        $wpdb->update($tablename, $companyemployee_data, array('user_id' => $user_id_update));
    } else {
        ...
        $wpdb->insert($tablename, $companyemployee_data);
         ...
         $message = urlencode('Welcome to CorpTnE');
         $mobile = $data['companyemployee']['txtempmob'];
