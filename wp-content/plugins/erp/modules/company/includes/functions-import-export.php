        ...
        foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
            ...
            for ($row = 2; $row <= $highestRow; $row++) {
                ...
                {  					
                    ...
                    $projectcode = $rowData[0]['ProjectCode'];
					$projectcode = trim($projectcode);
                    if($projectcode)
                    {
                            if($selProject=$wpdb->get_row("SELECT * FROM project_code WHERE PC_Code='$projectcode' AND COM_Id='$compid' AND PC_Active=1"))
                            {
                                    $projectcode=$selProject->PC_Id;
                            }
                            else
                            {                                                                  
								$wpdb->insert('project_code', array('COM_Id' => $compid,'PC_Code' => $projectcode));
                                $projectcode=$wpdb->insert_id;
						...
						$empError.=$empcode.", ";
						$array = array($empcode=>'Project Code doesnot Exists');
                    ...
                    if($grade)
                    {
                            ...
                            {
                                    $grade=$selGrade->EG_Id;
                            }
                            else
                            {                                                                  
                                 $wpdb->insert('employee_grades', array('COM_Id' => $compid,'EG_Name' => $grade,'EG_AddedBy' => $adminid));
                                   $grade=$wpdb->insert_id;
                    ...
                    //Cost center
                    $costcenter = $rowData[0]['CostCenter'];
                    $costcenter	= trim($costcenter);
                    if($costcenter)
                    {
                            if($selGrade=$wpdb->get_row("SELECT * FROM cost_center WHERE CC_Location='$costcenter' AND COM_Id='$compid' AND CC_Active=1"))
                            {
                                    $costcenter=$selGrade->CC_Id;
                            }
                            else
                            {                                                                  
                                 $wpdb->insert('cost_center', array('COM_Id' => $compid,'CC_Location' => $costcenter,'CC_AddedBy' => $adminid));
                                 $costcenter=$wpdb->insert_id;
					   ...
					   $empError.=$empcode.", ";
					   $array = array($empcode=>'Costcenter doesnot Exists');
					   $count++;
					}
                    $added=0;
                    if($empcode=="" || $name=="" || $email=="" || $username=="" || $dep=="" || $RMC=="" || $des=="" || $RFMC=="" || $grade=="" || $costcenter=="")
                    {
...
//                        }
                                ...
                                if($user_id  = wp_insert_user( $userdata )){
                                    ...
                                    if($ins=$wpdb->insert('employees', array('ADM_Id' => $adminid,'COM_Id' => $compid,'DEP_Id' => $dep,'DES_Id' => $des,
                                        'EMP_Code' => $empcode, 'CC_Id'=>$costcenter, 'ProjectId'=>$projectcode,'EMP_Email' => $email,'EMP_Username' => $username,'user_id' => $user_id,'EMP_Password' => $password,
                                        'EMP_Name' => $name,'EMP_Reprtnmngrcode' => $RMC,'EMP_Phonenumber' => $phn1,'EMP_Phonenumber2' => $phn2
                                        ,'EMP_Funcrepmngrcode' => $RFMC,'EG_Id' => $grade,'Added_Mode' => "1")))
                                    {
					    
					    if(isset($_POST['employee_notification'])){
					    	...
					    	$wpdb->insert('mail_notifications', array('ID' => $user_id,'FU_Id' => $fuid, 'EMP_Name' => $name, 'EMP_Username' => $username,'EMP_Email' => $email,'status' => 0));
					    }
