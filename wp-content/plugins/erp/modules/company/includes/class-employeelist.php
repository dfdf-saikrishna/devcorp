...
class Employeelist {
    ...
    private function get_erp_row( $force = false ) {
        ...
        if ( $this->id ) {
            ...
            if ( false === $row ) {
                 $query ="SELECT emp.*, adm.*,dep.*,des.*,eg.* 
					...
					LEFT JOIN employee_grades AS emp.EG_Id = eg.EG_Id
                    WHERE emp.EMP_Id = %d";	
    ...
    public function get_details_url() {
        if ( $this->id ) {
            return admin_url( 'admin.php?page=menu&action=view&id=' . $this->id );
        }
    }
