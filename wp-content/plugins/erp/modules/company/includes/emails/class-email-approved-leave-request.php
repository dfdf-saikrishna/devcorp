...
$mail_footer='</td>
	...
	$selempid=$selsql	=	$wpdb->get_row("SELECT re.EMP_Id FROM requests req, request_employee re WHERE REQ_Code='$requestcode' and req.REQ_Id=re.REQ_Id AND re.RE_Status=1 AND REQ_Active=1");
	...
	$totAmount		=	$wpdb->get_row("SELECT SUM(rd.RD_Cost) AS total FROM requests req, request_details rd WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=rd.REQ_Id AND RD_Status=1 AND REQ_Active != 9 ");
	...
	$reqDetails="<BR><table width=\"98%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" style=\"font-size:12px;font-weight: bold;\">
				  ...
				  <tr>
					...
					<td colspan=\"6\">:</td>
					<td width=\"60%\" align=\"left\" valign=\"middle\">".IND_money_format($totAmount->total).".00"."</td>
				  </tr>
				</table>";
	...
	switch($type)
	{
		...
		case 31: //mail to accounts
		$mailsub	=	$ettype;
		$name		=	"To Finance";
		$text		=	"The requested employee is the functional reporting manager itself.<br>Approval waiting from the Finance.";
		$text.=$reqDetails;
		break;
		...
		$message    =   urlencode('You have an expense request waiting for your approval');
		$msgto		=	$repmngr->EMP_Phonenumber;
		$name		=	"Hi ".$repmngr->EMP_Name;
		$text		=	"<br>You have an expense request waiting for your approval.<br>";
		$text.=$reqDetails;
		break;
		
		case 30: //mail to functional reporting manager
		$mailsub	=	$ettype;
		$repmngr	=	$wpdb->get_row("SELECT * FROM employees WHERE EMP_Code='$empdetails->EMP_Funcrepmngrcode'");
		$mailto		=	$repmngr->EMP_Email;
		$message    =   urlencode('You have an expense request waiting for your approval');
		$msgto		=	$repmngr->EMP_Phonenumber;
		$name		=	"Hi ".$repmngr->EMP_Name;
		...
		$message    =   urlencode('The expense request have been approved by your reporting manager & have been sent to finance for further verifications');
		$msgto		=	$employee->EMP_Phonenumber;
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The expense request have been rejected by your reporting manager.<br>Please revise your expense request & submit later');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The expense request has been approved by the Finance');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		if($et==1){
			...
			$message.="You are allowed to make the journey";	
		}
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The expense request has been rejected by the Finance.You may please revise your request');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The expense request have been approved by Finance & have been sent to your reporting manager for further verification');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$repmngr	=	$wpdb->get_row("SELECT * FROM employees WHERE EMP_Code='$empdetails->EMP_Reprtnmngrcode'");
		$mailto		=	$repmngr->EMP_Email;
		$message    =   urlencode('The expense requested has been submitted for claim. Waiting for claim approval');
		$msgto		=	$repmngr->EMP_Phonenumber;
		$name		=	"Hi ".$repmngr->EMP_Name;
		...
		$repmngr	=	$wpdb->get_row("SELECT * FROM employees WHERE EMP_Code='$empdetails->EMP_Reprtnmngrcode'");
		$mailto		=	$repmngr->EMP_Email;
		$message    =   urlencode('The request has been updated for claim. Waiting for claim approval');
		$msgto		=	$repmngr->EMP_Phonenumber;
		$name		=	"Hi ".$repmngr->EMP_Name;
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The expense request claim have been approved by your reporting manager & have been sent to finance for further verifications');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The expense request claim have been rejected by your reporting manager.Please revise your expense request"s claim & submit later');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The expense request claim has been approved by Finance');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The above mentioned expense request claim has been rejected by Finance.You may please revise your request and submit later');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The expense request has been Auto-Approved');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$message    =   urlencode('The above mentioned expense request payment details have been updated by the finance department. Please verify payment details');   
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$message    =   urlencode('payment details have been added by the finance department. Please verify payment details');
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
		...
		$employee	=	$wpdb->get_row("SELECT * FROM requests req, request_employee re, employees emp WHERE req.REQ_Code='$requestcode' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id=emp.EMP_Id AND RE_Status=1");
		$mailto		=	$employee->EMP_Email;
		$message    =   urlencode('The expense request have been approved by your reporting manager. You can make your journey');
		$msgto		=	$employee->EMP_Phonenumber;
		$name		=	"Hi ".$employee->EMP_Name;
					  ...
					  <tr>
						<td height="34"><table width="100%" border="0" cellspacing="0" cellpadding="0">
							<tr>
							  <td width="3%"><img src="'.$assetimgpath.'BusinessSimple_01.gif" width="17" height="104"></td>
							  <td width="25%" background="'.$assetimgpath.'BusinessSimple_08.gif"><a href="'.$domain.'"><img src="'.$assetimgpath.'logo_invice.png" ></a></td>
							  <td width="68%" align="center" valign="middle" background="'.$assetimgpath.'BusinessSimple_08.gif" style="font-size:24px;color:#443A8F;padding-left:9px;background-repeat: repeat-x;line-height: 98px;height: 98px;"><img src="'.$domain.'admin/'.$empdetails->COM_Logo.'"></td>
							  <td width="4%"><img src="'.$assetimgpath.'BusinessSimple_03.gif" width="27" height="104"></td>
							</tr>
						  </table></td>
					  </tr>
		...
		case 1:case 4: case 10: case 14: case 20: case 21: // MAIL TO THE FINANCE APPROVERS
			...
			foreach($selacc as $rowsql){
				...
				//mail($mail_to,$mail_sub,$mail_mesg,$headers);
					$this->sendsms( $rowsql->EMP_Phonenumber, $message );
			}
