...
class Upload_List_Table extends \WP_List_Table {
    ...
    function column_download($item) {
        ...
        global $wpdb;
        $fuId = $item['FU_Id'];
        $pending_count = count($wpdb->get_results("SELECT * FROM mail_notifications WHERE FU_Id = '$fuId'"));
        if($pending_count)
         echo "<a href='".WPERP_COMPANY_DOWNLOADS.$fileurl."' download='import_file'>Download</a> " . " <input type='button' class='button-default' value='Send Notification' fuId = $fuId id='send_notification'>";
        else
    ...
    function column_notifications($item) {
        ...
        $pending_count = count($wpdb->get_results("SELECT * FROM mail_notifications WHERE FU_Id = '$fuId'"));
        if($pending_count)
         echo '<span class="status-1">Pending</span>' . ' - ' . '('.$pending_count.') employees';
        else
         if ($item['FU_Status'] == 1)
            echo '<span class="status-3">N/A</span>';
         else
            echo '<span class="status-2">Sent</span>';
    }
    ...
    function get_columns() {
        $columns = array(
           ...
           //'file' => __('File Name', 'costcenter_table_list'),
            ...
            'notifications' => __('Notifications', 'costcenter_table_list'),
