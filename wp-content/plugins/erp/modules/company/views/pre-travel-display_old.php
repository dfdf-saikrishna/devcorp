...
$empdetails = $wpdb->get_row("SELECT * FROM employees emp, company com, department dep, designation des,requests req, request_employee re, employee_grades eg WHERE req.REQ_Id='$reqid'  AND req.REQ_Id=re.REQ_Id AND emp.EMP_Id=re.EMP_Id  AND emp.COM_Id=com.COM_Id AND emp.DEP_Id=dep.DEP_Id AND emp.DES_Id=des.DES_Id AND emp.EG_Id=eg.EG_Id AND req.REQ_Active=1 AND re.RE_Status=1");

//if (isset($empdetails)) {
//    $code = $empdetails->EMP_Reprtnmngrcode;
//}
...
//$selsql =$wpdb->get_row("SELECT * FROMrequest_details rd, expense_category ec, mode mot", "*", "rd.REQ_Id=$row[REQ_Id] AND rd.EC_Id=ec.EC_Id AND rd.MOD_Id=mot.MOD_Id AND rd.RD_Status=1 ORDER BY rd.RD_Dateoftravel ASC", $filename, 0);
//print_r($row);
                    ...
                    <table class="wp-list-table widefat striped admins" border="0" id="table1">
                        <thead class="cf">
                            <tr>
                                ...
                                <th class="column-primary">Expense <br/> Description</th>
                        ...
                        <tbody>
                            ...
                            foreach ($selsql as $rowsql) {
                                ...
                                <tr>
                                    ...
                                    <td data-title="Category"><?php echo $rowsql->EC_Name; ?><br/><?php echo $rowsql->MOD_Name; ?></td>
                                        ...
                                        <td><?php
                                                    ...
                                                    if ($selrdbs = $wpdb->get_row("SELECT * FROM booking_status WHERE RD_Id='$rowsql->RD_Id' AND BS_Status=2 AND BS_Active=1")) {
                                                        ...
                                                        if (!empty($selrdbs->RD_Id)) {
                                                            if ($selrdbs->RD_Id) {
                                                                ...
                                                                $seldocs = $wpdb->get_row("SELECT * FROM booking_documents WHERE BS_Id='$bsId'");
                                            ...
                                            if ($row->REQ_Status == 2) {
                                                ...
                                                if (in_array($rowsql->MOD_Id, array(1, 2, 5))) {
                                                    ...
                                                    if ($selrdbs = $wpdb->get_row("SELECT * FROM booking_status WHERE RD_Id = '$rdId' AND BS_Status = 2 AND BS_Active = 1")) {

                                                        echo bookingStatus(NULL);
                                                    } else {

                                                        $selrdbs = $wpdb->get_row("SELECT * FROM booking_status WHERE RD_Id = '$rdId' AND BS_Status = 3 AND BS_Active = 1");
                                                        //print_r($selrdbs);die;
                                                        //$rdbId=$selrdbs->RD_Id;
                                                        if (!empty($selrdbs->RD_Id)) {
                                                            if ($selrdbs->RD_Id) {
                                                                ...
                                                                if ($selrdbs->BA_Id == 1) {
                                                                    ...
                                                                    $seldocs = $wpdb->get_row("SELECT * FROM booking_documents", "*", "BS_Id = '$selrdbs[BS_Id]'");
                                                    ...
                                                    <div id="quoteDivId">
                                                        ...
                                                        foreach ($rdidarry as $rdid) {
                                                            ...
                                                            if (count($selrgquote)) {
                                                                ...
                                                                if ($rowRdDetails = $wpdb->get_row("SELECT * FROM request_details rd, mode mo WHERE RD_Dateoftravel, MOD_Name, RD_Cityfrom, RD_Cityto, rd.MOD_Id, SD_Id", "RD_Id=$rdid AND rd.MOD_Id=mo.MOD_Id")) {
                                                                    ?>
