...
$cancelstatus = '<table class="hide-table'.$item['REQ_Id'].' init-invoice wp-list-table widefat fixed striped collapse"><th>Cancel Status</th><tr><td>' . $cancellationstatus .'</td></tr></table>';
...
$cancellationstatus="";
	...
	$getvals = $wpdb->get_results("SELECT DISTINCT (rd.RD_Id),rd.*,bs.* FROM request_details rd,booking_status bs WHERE rd.REQ_Id='".$item['REQ_Id']."' AND rd.RD_Id=bs.RD_Id AND bs.BS_Status IN (1,3) AND BS_Active=1 AND RD_Status=1");
	 ...
	 if($icon==1){
        ...
        //$bookingstatuss = '<table class="hide-table'.$item['REQ_Id'].' init-invoice wp-list-table widefat fixed striped collapse"><th>Booking Status</th><tr><td>'. $_GET['res'] .'</td></tr></table>';
           //header("location:/wp-admin/admin.php?page=traveldesk-dashboard");exit;    
