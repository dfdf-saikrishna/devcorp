	...
	for(var i=0;i<chks.length;i++)
	{
		...
		if(reg.test(costcont)){              
			...
			var cnt = $j("#selEmployees").val();
                        var cnt = cnt.length;
			if(cnt && chks[i].value)
			document.getElementById("txtCost"+(i+1)).value = chks[i].value * cnt;
			//alert(document.getElementById("txtCost"+(i+1)).value = chks[i].value * cnt);
			getGrpBookingTotal();
...
function getGrpBookingTotal()
{
	...
	if(totalcost!=0 && totalcost!=""){
		...
		document.getElementById('totaltable').innerHTML='<table class="table" style="font-weight:bold;"><tr ><td align="right" colspan="8" style="text-align:right;font-weight:bold;">Total Cost</td><td align="right" colspan="" width="10%" style="font-weight:bold;">Rs '+totalcost+'.00</td></tr></table>';
		}else{
