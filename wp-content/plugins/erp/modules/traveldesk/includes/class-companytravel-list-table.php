 ...
 */
class Companiestravel_List_Table extends \WP_List_Table
{
    /**
...
//        }
    ...
    function column_name($item)
    {
        ...
        $actions = array(
            'edit' => sprintf('<a href="?page=travel-dashboard" data-id=%s>%s</a>', $item['TD_Id'], __('Edit', 'companiestravel_table_list')),
            'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['TD_Id'], __('Delete', 'companiestravel_table_list')),
        );
        
		return sprintf('%s %s ',
            '<a href="'.erp_company_url_single_companyview( $item['COM_Id']).'"><strong>' . $item['TD_Username'] . '</strong></a>',
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'name' => __('User Name', 'companies_table_list'),
            'email' => __('User Email', 'companies_table_list'),
            'Created_Date' => __('Added On Date', 'companies_table_list'),
        );
        return $columns;
    }
    ...
    {
        global $wpdb;
        $table_name = 'travel_desk'; // do not forget about tables prefix
		...
		if(!empty($_POST["s"])) {
			...
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE COM_Id=1".$query."ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
		else{
