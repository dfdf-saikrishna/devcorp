...
class Companiestravelinvoice_List_Table extends \WP_List_Table
{
    /**
...
//        }
    ...
    {
        //$reqs=count_query("travel_desk_claim_requests","TDCR_Id","WHERE TDC_Id='$rowsql[TDC_Id]'",$filename, 0);
        return '<em>' . $item['TDCR_Quantity'] . '</em>';
    }
    
    function column_Created_Date($item){

        return date('d/M/Y', strtotime($item['TD_Addeddate'])) . date('h:i a', strtotime($item['TD_Addeddate']));
    }
    ...
    function column_reference($item)
    {
        ...
        $actions = array(
            //'edit' => sprintf('<a href="?page=travel-dashboard" data-id=%s>%s</a>', $item['TD_Email'], __('Edit', 'companies_table_list')),
            //'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['COM_Id'], __('Delete', 'custom_table_example')),
        );
        
		return sprintf('%s %s ',
            '<a href="'.erp_company_url_single_companyview( $item['COM_Id']).'"><strong>' . $item['TDC_ReferenceNo'] . '</strong></a>',
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'reference' => __('Reference No.', 'companies_table_list'),
            'requests' => __('Requests', 'companies_table_list'),
            'quantity' => __('Quantity', 'companies_table_list'),
            'amount' => __('Total Amount (Rs)', 'companies_table_list'),
            'status' => __('Claim Status', 'companies_table_list'),
            'invoicedate' => __('Invoice Date', 'companies_table_list'),
        );
        return $columns;
    }
    ...
    function prepare_items()
    {
		...
		if(!empty($_POST["s"])) {
			...
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE COM_Id=0".$query."ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
		else{
                        $comId = $_SESSION['compid'];
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM travel_desk_claims,travel_desk_claim_requests WHERE COM_Id='$comId' ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
