...
class Superadmin_List_Table extends \WP_List_Table
{
    ...
    function __construct()
    {
        ...
        parent::__construct(array(
            'singular' => 'request_details',
            'plural' => 'request_detailss',
        ));
    }
     ...
     * @return HTML
     */
    function column_rt($item)
    {
        return '<em>' . $item['REQ_Type'] . '</em>';
    }
    
    function column_quantity($item){
        return $item['TDCR_Quantity'];
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'rc' => __('Request Code', 'custom_table_example'),
            'rt' => __('Request Type', 'custom_table_example'),
            'quantity' => __('Quantity', 'custom_table_example'),
            'date' => __('Date', 'custom_table_example'),
            'amount' => __('Quote Amount (Rs)', 'custom_table_example'),
    ...
    {
        global $wpdb;
        $table_name = 'request_details'; // do not forget about tables prefix
        $table_name1 = 'booking_status';    
        $table_name2 = 'requests';
        $table_name3 ='travel_desk_claim_requests';
        $table_name4 ='travel_desk_claims';
        ...
        $total_items = $wpdb->get_var("SELECT COUNT(REQ_Id) FROM $table_name");

        // prepare query params, as usual current page, order by and order direction
        $paged = isset($_REQUEST['paged']) ? max(0, intval($_REQUEST['paged']) - 1) : 0;
        $paged = $paged * $per_page;
        $orderby = (isset($_REQUEST['orderby']) && in_array($_REQUEST['orderby'], array_keys($this->get_sortable_columns()))) ? $_REQUEST['orderby'] : 'REQ_Code';
		...
		if(!empty($_POST["s"])) {
			...
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name,$table_name1,$table_name2,$table_name3,$table_name4 WHERE $table_name.RD_Id=$table_name1.RD_Id AND $table_name1.BS_Status=3 AND BS_Active=1 AND RD_Status=1 AND $table_name.REQ_Id=$table_name2.REQ_Id AND $table_name4.COM_Id='$compid' and $table_name4.TDC_Id=$table_name3.TDC_Id  ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
