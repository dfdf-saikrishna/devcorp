...
function get_traveldesk_list(){
	...
	$companylist = $wpdb->get_results( "SELECT EMP_Id, EMP_Code, EMP_Name FROM  employees WHERE COM_Id='$compid' AND EMP_Status=1 ORDER BY EMP_Name ASC");
	return $companylist;
}
function get_traveldeskemp_list(){
	global $wpdb;
        $compid = $_SESSION['compid'];
        ///$empid = $_SESSION['empid'];
	$companylist = $wpdb->get_results( "SELECT EMP_Id, EMP_Code, EMP_Name FROM  employees emp, company com, department dep, designation des, employee_grades eg WHERE emp.EMP_Id='1' AND com.COM_Id='48' AND emp.COM_Id=com.COM_Id ");
	return $companylist;
}
 ...
 */
function travel_create( $args = array() ) {
    global $wpdb;
    $defaults = array(
        //'user_email'      => '',
        'company'        => array(
 ...
 */
function travel_get( $args = array() ) {
    global $wpdb;
