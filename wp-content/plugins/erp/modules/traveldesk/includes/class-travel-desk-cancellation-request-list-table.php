...
function extra_tablenav($which) {
	...
	<div class="alignleft actions">
        <a href="#" id="traveldeskrise_invoice" class="button button-primary">Raise Invoice</a> 
        </div>
		<?php
}
...
$getvals = $wpdb->get_results("SELECT DISTINCT (rd.RD_Id),rd.*,bs.* FROM request_details rd,booking_status bs WHERE rd.REQ_Id='".$item['REQ_Id']."' AND rd.RD_Id=bs.RD_Id AND bs.BS_Status IN (1,3) AND BS_Active=1 AND RD_Status=1");
			...
			$rddetails = $wpdb->get_results("SELECT * FROM request_details rd, expense_category ec, mode mo WHERE REQ_Id='".$item['REQ_Id']."' AND rd.RD_Id IN ($rdids) AND rd.EC_Id=ec.EC_Id AND rd.MOD_Id=mo.MOD_Id AND RD_Status=1 ORDER BY RD_Id ASC");
                        ...
                        echo '<span  width="15%" title="'. $title .'"><a href="'. $href .'">'. $item['REQ_Code'] . $icon .'</span></a>';
						if(!empty($rddetails)){
                    ...
                    if(!empty($date))
                    $reqdate = '<table class="hide-table'.$item['REQ_Id'].' init-invoice wp-list-table widefat fixed striped collapse"  style="font-size:11px;"><th>Date</th><tr><td>'. $date .'</td></tr></table>';
                    else
                    $reqdate ='';
                    echo $reqdate;
                    echo '</br>';
...
$getvals = $wpdb->get_results("SELECT DISTINCT (rd.RD_Id),rd.*,bs.* FROM request_details rd,booking_status bs WHERE rd.REQ_Id='".$item['REQ_Id']."' AND rd.RD_Id=bs.RD_Id AND bs.BS_Status IN (1,3) AND BS_Active=1 AND RD_Status=1");
				...
				if($void){
					...
					//echo 'tdc status='.$selclmreqid[TDC_Status]."<br>";
                        ...
                        $EC_Name = '<table class="hide-table'.$item['REQ_Id'].' init-invoice wp-list-table widefat fixed striped collapse"  style="font-size:11px;"><th>Expense 
...
$getvals = $wpdb->get_results("SELECT DISTINCT (rd.RD_Id),rd.*,bs.* FROM request_details rd,booking_status bs WHERE rd.REQ_Id='".$item['REQ_Id']."' AND rd.RD_Id=bs.RD_Id AND bs.BS_Status IN (1,3) AND BS_Active=1 AND RD_Status=1");
				...
				if($void){
					...
					//echo 'tdc status='.$selclmreqid[TDC_Status]."<br>";
						...
						$rddetails = $wpdb->get_results("SELECT * FROM request_details rd, expense_category ec, mode mo WHERE REQ_Id='". $item['REQ_Id'] ."' AND rd.RD_Id IN ($rdids) AND rd.EC_Id=ec.EC_Id AND rd.MOD_Id=mo.MOD_Id AND RD_Status=1 ORDER BY RD_Id ASC");
				        echo '<a data-id="'.$item['REQ_Id'].'" class="traveldeskrequestarrow" data-toggle="collapse" href="#collapse"><i class="collapse-caret fa fa-angle-down"></i> </a>'; 
						if(!empty($rddetails)){
						foreach ($rddetails as $rowsql) {
				if ($selrdcs = $wpdb->get_row("SELECT * FROM booking_status WHERE RD_Id='". $rowsql->RD_Id ."' AND BS_Status=3 AND BS_Active=1")) {
				$j = $selrdcs->RD_Id;						
                                    ?>
								<form method="post" id="cancellationForm<?php echo $j; ?>" name="cancellationForm<?php echo $j; ?>">
							...
							switch ($selrdcs->BA_Id) {
                                ...
                                <div class="col-sm-8">
                                  <div class="form-group">
                                    <div>
									<?php $cancellationstatus .='<form action="#" method="post" enctype="multipart/form-data">';?>
                                    
                                     <?php $cancellationstatus.= '<select name="selCancActions'.$j.'" id="selCancActions'.$j.'" class="form-control" onChange="showHideCanc('. $j .', this.value)">
                                       <option value="">Select</option>'; 
                                           $ba = $wpdb->get_results("Select * FROM booking_actions WHERE $query");
											foreach ($ba as $barows) {
                                         
                                       $cancellationstatus.=  '<option value="'. $barows->BA_Id .'">'. $barows->BA_Name .'</option>'; ?>
                                        <?php } 
                                      $cancellationstatus.=  '</select>';?>
                                    </div>
                                  </div>
								  <?php $cancellationstatus .=  '<div id="buttonShow1'.$j.'"></div>'; ?>
                                      
                                </div>
                                <div class="clearfix"></div>
                                <?php  $cancellationstatus .='</form>';?>
                                <?Php } ?>
                              </div>
                            </form>		
						<?php } else {
									...
									echo '<table class="hide-table'.$item['REQ_Id'].' init-invoice wp-list-table widefat fixed striped collapse"><th>Cancel Status</th><tr><td>' . $cancellationstatus .'</td></tr></table>';
...
$getvals = $wpdb->get_results("SELECT DISTINCT (rd.RD_Id),rd.*,bs.* FROM request_details rd,booking_status bs WHERE rd.REQ_Id='".$item['REQ_Id']."' AND rd.RD_Id=bs.RD_Id AND bs.BS_Status IN (1,3) AND BS_Active=1 AND RD_Status=1");
	...
	$rddetails = $wpdb->get_results("SELECT * FROM request_details rd, expense_category ec, mode mo WHERE REQ_Id='$item[REQ_Id]' AND rd.RD_Id IN ($rdids) AND rd.EC_Id=ec.EC_Id AND rd.MOD_Id=mo.MOD_Id AND RD_Status=1 ORDER BY RD_Id ASC");
				        echo  '<span class="status-2" style="padding:5px 8px !important;border-radius: 15px;line-height:1">'. count($getvals) .'</span>'; 
						if(!empty($rddetails)){
						foreach ($rddetails as $rowsql) {	
						 if ($rowsql->EC_Id == 1) {
					...
					if ($rowsd = $wpdb->get_results("SELECT SD_Name FROM stay_duration WHERE SD_Id='".$rowsql->SD_Id ."'")) {
						...
						if(!empty($Loc))
                        $Loc = '<table class="hide-table'.$item['REQ_Id'].' init-invoice wp-list-table widefat fixed striped collapse"><th>Place</th><tr><td>'. $Loc .'</td></tr></table>';
                        else
                        $Loc ='';
                        echo $Loc;
						}
...
$bookingStatus="";
	...
	$getvals = $wpdb->get_results("SELECT DISTINCT (rd.RD_Id),rd.*,bs.* FROM request_details rd,booking_status bs WHERE rd.REQ_Id='".$item['REQ_Id']."' AND rd.RD_Id=bs.RD_Id AND bs.BS_Status IN (1,3) AND BS_Active=1 AND RD_Status=1");
			...
			$j = $selrdbs->RD_Id;
                        if ($selrdbs->RD_Id) {
                                                                        ?>
                            <form method="post" id="bookingForm<?php echo $j; ?>" name="bookingForm<?php echo $j; ?>" >
						...
						case 3:
							...
							break;
                                ...
                                <div class="col-sm-8" >
                                  <div class="form-group">
                                    <div>
                                        <?php $bookingStatus .='<form action="#" method="post" enctype="multipart/form-data">';?>
                                    <?php $bookingStatus .=  '<select name="selBookingActions'.$j.'" id="selBookingActions'.$j.'" class="form-control" onchange="showHideBooking('.$j.', this.value)" >
                                      ...
                                      <?php $bookingStatus .=  '<option value="'. $barows->BA_Id .'">'. $barows->BA_Name .'</option>'; ?>
                                        <?php } ?>
                                      <?php $bookingStatus .=  '</select><div id="buttonShow'.$j.'"></div>'; ?>
                                      
                                    </div>
                                  </div>
                                </div>
                                <div class="clearfix"></div>
                                <?php  $bookingStatus .='</form>';?>
                              <?php  } ?>
                              </div>
...
function column_Date($item) {
	...
	$rddetails = $wpdb->get_results("SELECT * FROM request_details rd, expense_category ec, mode mo WHERE REQ_Id='". $item['REQ_Id'] ."' AND rd.RD_Id IN ($rdids) AND rd.EC_Id=ec.EC_Id AND rd.MOD_Id=mo.MOD_Id AND RD_Status=1 ORDER BY RD_Id ASC");
			            echo date('d-M-Y', strtotime($item['REQ_Date']));
						if(!empty($rddetails)){
						foreach ($rddetails as $rowsql) {	
						$RD_Cost = IND_money_format($rowsql->RD_Cost) . ".00";
						if(!empty($RD_Cost))
                        $RD_Cost = '<table class="hide-table'.$item['REQ_Id'].' init-invoice wp-list-table widefat fixed striped collapse"><th>Estimate</th><tr><td>'. $RD_Cost .'</td></tr></table>';
                        else
                        $RD_Cost ='';
						echo $RD_Cost;
						}
...
'nbsp' => __('Details', 'companyinvoicecreate_table_list'),
);
return $columns;
}
