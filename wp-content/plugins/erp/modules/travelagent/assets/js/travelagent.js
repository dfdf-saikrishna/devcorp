...
;(function($) {
    ...
    var WeDevs_ERP_TRAVELAGENT = {
        ...
        initialize: function() {
            ...
            $( 'body' ).on( 'click', '#travelagentrise_invoice', this.travelagentInvoice.raiseInvoice );
			...
			$('body').on('keyup', '.calcInvoiceAmnt', this.travelagent.calcInvoiceAmount);
            ...
            calcInvoiceAmount: function () {
            	...
            	if(servTax || servChrg){
            		...
            		if(servTax){
            			...
            			var calcText	=	WeDevs_ERP_TRAVELAGENT.travelagent.indianRupeeFormat(calc);
            		...
            		totalAmntFrmtd=WeDevs_ERP_TRAVELAGENT.travelagent.indianRupeeFormat(totalAmnt)+'.00';
	...
	travelagentClient: {
                        ...
                        wp.ajax.send('travelagentclient_get', {
                            ...
                            success: function(response) {
                              ...
                              var html = wp.template('travelagentclient-create')( response );
                                ...
                                $( 'li[data-flight]', modal ).each(function() {
                                    var self = $(this),
                                        flight = self.data('flight');
                                    if ( flight !== '' ) {
                                        $('input:radio[name="company_markups_markdowns[radioFlightMarkStatus]"][value="'+flight+'"]').attr('checked', true);
                                    }
                                });
                                $( 'li[data-bus]', modal ).each(function() {
                                    var self = $(this),
                                        bus = self.data('bus');
                                    if ( bus !== '' ) {
                                        $('input:radio[name="company_markups_markdowns[radioBusMarkStatus]"][value="'+bus+'"]').attr('checked', true);
                                    }
                                });
                                $( 'li[data-hotel]', modal ).each(function() {
                                    var self = $(this),
                                        hotel = self.data('hotel');
                                    if ( hotel !== '' ) {
                                        $('input:radio[name="company_markups_markdowns[radioHotelMarkStatus]"][value="'+hotel+'"]').attr('checked', true);
                                    }
                                });
		...
		travelagentInvoice: {
            ...
            raiseInvoice: function(e) {
                ...
                var cmpid = $('#cmpid').val();
                if(values!=""){
				window.location.replace("/wp-admin/admin.php?page=RiseInvoice&action=view&reqid=" + values + "&cmpid=" + cmpid);
				}
            },
