 ...
 */
class Travel_Agent_Client_List_Table extends \WP_List_Table
{
    /**
	...
	function column_bookingreq($item){
        ...
        return sprintf('%s %s %s',
            '',
            '<a href="'.erp_travelagent_requestview( $item['COM_Id'],1).'"><span class="oval-1"><strong>' . getCountRequests(1, $item['COM_Id']) . '</strong></span></a>',''
        );
    }
	...
	function column_cancelreq($item){
        ...
        return sprintf('%s %s %s',
            '',
            '<a href="'.erp_travelagent_requestview( $item['COM_Id'],2).'"><span class="oval-4"><strong>' . getCountRequests(2, $item['COM_Id']) . '</strong></span></a>',''
        );
    }
	...
	function column_totbookingreq($item){
        ...
        return sprintf('%s %s %s',
            '',
            '<a href="'.erp_travelagent_requestview( $item['COM_Id'],3).'"><span class="oval-2"><strong>' . getCountRequests(3, $item['COM_Id']) . '</strong></span></a>',''
        );
    }
	...
	function column_totcancelreq($item){
        ...
        return sprintf('%s %s %s',
            '',
            '<a href="'.erp_travelagent_requestview( $item['COM_Id'],4).'"><span class="oval-4"><strong>' . getCountRequests(4, $item['COM_Id']) . '</strong></span></a>',''
        );
    }
    /**
    ...
    function get_columns()
    {
        $columns = array(
            ...
            'bookingreq' => __('New <br> Booking Requests', 'travelagentclient_table_list'),
