...
class Admin_Menu {
    ...
    add_menu_page('agent Dashboard', 'Travelagent DashBoard', 'travelagent', 'dashboard', array( $this, 'dashboard_page'),'dashicons-dashboard');  
   ...
   add_menu_page('Client', 'Client Management', 'travelagent','ClientM', array( $this, 'travel_agent_client_listing'),'dashicons-admin-users');
        ...
        add_submenu_page('', 'request', 'RequestDetails', 'travelagent', 'RequestDetails', array($this, 'travel_agent_request_details'));
        add_submenu_page('', 'request', 'GroupRequestDetails', 'travelagent', 'GroupRequestDetails', array($this, 'travel_agent_group_request_details'));

        
    add_menu_page('InvoiceM', 'Invoice Management', 'travelagent','InvoiceM', array( $this,'company_invoicecreate'),'dashicons-id-alt');
		$overview = add_submenu_page( 'InvoiceM', 'Overview', 'Overview', 'travelagent', 'InvoiceM', array( $this,'company_invoicecreate'));
		add_submenu_page('', 'Create', 'Create Invoice', 'travelagent', 'createinvoice', array( $this,'company_invoicemanagement'));
		 add_submenu_page('', 'ViewInv', 'User Invoices ', 'travelagent', 'invoicecreate', array( $this,'user_invoices'));
		 ...
		 switch ($action) {
            case 'view':
                $riseinvoiceview = new Riseinvoiceview($id);
                 if ( !$id ) {
                    wp_die( __( 'Request Id not found!', 'erp' ) );
                }
    ...
    public function travel_agent_request_details(){
        include WPERP_TRAVELAGENT_VIEWS . '/travel-agent-request-details.php';
    }
    
    public function travel_agent_group_request_details(){
        include WPERP_TRAVELAGENT_VIEWS . '/travel-agent-group-request-details.php';
    }
    ...
    public function user_invoices(){
        include WPERP_TRAVELAGENT_VIEWS . '/travelagent/travelagentuserinvoice.php';
    }
    
    public function rise_invoice(){
        //include WPERP_TRAVELAGENT_VIEWS . '/travelagent/travelagentriseinvoice-create.php';
    }
