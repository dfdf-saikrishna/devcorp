...
//alert($posted);
    ...
    public function travelagentclient_create() {
        ...
        // user notification email
            ...
            $emailer    = wperp()->emailer->get_email( 'New_Employee_Welcome' );
            $send_login = isset( $posted['login_info'] ) ? true : false;
            if($travelagentclient_id!="User email already Exists"){
                if ( is_a( $emailer, '\WeDevs\ERP\Email') ) {
	...
	public function travelagentclient_get() {
        ...
        //Flight
        $flightmarkup = $wpdb->get_row("SELECT CMM_Id, MOD_Name, cmm.MC_Id, MC_Name, CMM_MarkFare, CMM_MarkStatus, CMM_Fromdate, CMM_Todate, CMM_Status FROM company_markups_markdowns cmm, mode mo, markupdown_category mc WHERE cmm.COM_Id=$id AND cmm.MOD_Id = 1 AND CMM_Status = 1 AND cmm.MOD_Id = mo.MOD_Id AND cmm.MC_Id = mc.MC_Id");
        $flightmarkup = array('flight' => $flightmarkup);
        
        //Bus
        $busmarkup = $wpdb->get_row("SELECT CMM_Id, MOD_Name, cmm.MC_Id, MC_Name, CMM_MarkFare, CMM_MarkStatus, CMM_Fromdate, CMM_Todate, CMM_Status FROM company_markups_markdowns cmm, mode mo, markupdown_category mc WHERE cmm.COM_Id=$id AND cmm.MOD_Id = 2 AND CMM_Status = 1 AND cmm.MOD_Id = mo.MOD_Id AND cmm.MC_Id = mc.MC_Id");
        $busmarkup = array('bus' => $busmarkup);
        
        //Hotel
        $hotel = $wpdb->get_row("SELECT CMM_Id, MOD_Name, cmm.MC_Id, MC_Name, CMM_MarkFare, CMM_MarkStatus, CMM_Fromdate, CMM_Todate, CMM_Status FROM company_markups_markdowns cmm, mode mo, markupdown_category mc WHERE cmm.COM_Id=$id AND cmm.MOD_Id = 5 AND CMM_Status = 1 AND cmm.MOD_Id = mo.MOD_Id AND cmm.MC_Id = mc.MC_Id");
        $hotel = array('hotel' => $hotel);
        
        $obj_merged = (object) array_merge((array) $flightmarkup, (array) $busmarkup, (array) $hotel, (array) $response);
